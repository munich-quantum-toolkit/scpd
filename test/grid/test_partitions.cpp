...
std::vector<PartitionLabel> splitLeftAndRight(const GridMetrics& grid, const std::uint32_t at) {
  ...
  for (std::uint32_t y = 0; y < grid.height; ++y) {
    for (std::uint32_t x = 0; x < grid.width; ++x) {
      labels[grid.index(x, y)] = x < at ? FIRST_PARTITION_LABEL
...
TEST(Partitions, FindsTheBorderBetweenTwoLabelsAndItsLength) {
  ...
  const auto partitions = extractPartitions(free, splitLeftAndRight(grid, 4), grid);
...
TEST(Partitions, TreatsAnObstacleAsNoLabelAtAll) {
  ...
  const auto partitions = extractPartitions(mask, splitLeftAndRight(grid, 4), grid);
...
TEST(Partitions, PutsOneLatticePointOnEachDistinctBorderSample) {
  ...
  EXPECT_EQ(partitions.lattice.size(), partitions.borders.front().samples.size());
  // The points come back in a fixed order, so the lattice a router builds
  // from them is the same on every run.
  EXPECT_TRUE(std::ranges::is_sorted(partitions.lattice, [](const auto& a, const auto& b) {
...
TEST(Partitions, RefusesLabelsThatDoNotFitTheGrid) {
  ...
  EXPECT_THROW(static_cast<void>(extractPartitions(free, tooFew, grid)), std::invalid_argument);
}
...
TEST(FreeCellSeeds, RefusesADetailGridThatIsNoWholeRefinement) {
  ...
  EXPECT_THROW(static_cast<void>(freeCellSeeds(mask, detail, coarse)), std::invalid_argument);
}
...
TEST(CellCapacities, BudgetsAFreeEdgeByTheCellExtentOverThePitch) {
  ...
  const auto coarse = GridMetrics::fit({.minX = 0.0, .minY = 0.0, .maxX = 400.0, .maxY = 400.0},
                                       4, 4);
  const auto detail = coarse.refined(5);
