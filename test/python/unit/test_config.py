...
WITH_MATING_AND_GRID = (
    MANUAL.replace(
        "conventional = '^(Qb\\d+\\.port1|Coupler\\d+_\\d+\\.port[0-4])$'\n",
        "conventional = '^(Qb\\d+\\.port1|Coupler\\d+_\\d+\\.port[0-4])$'\nmating = '^Qb\\d+\\.port[2-5]$'\n",
    )
    + """
[grid]
capacity_cells_x = 25
...
def test_manual_configuration_loads_with_the_schema_defaults() -> None:
    ...
    assert config.ports.patterns.mating is None
...
def test_the_mating_pattern_and_the_grid_are_optional() -> None:
    """An optional mating pattern and a partial grid section load with the defaults for the rest."""
    config = parse_config(WITH_MATING_AND_GRID)

    assert config.ports is not None
    assert config.ports.patterns is not None
    assert config.ports.patterns.mating == r"^Qb\d+\.port[2-5]$"
...
def test_shipped_configurations_carry_only_what_differs_from_the_defaults() -> None:
    ...
    problems = shipped_config_problems({
        "ports": {"patterns": {"mating": ""}},
        "grid": {"capacity_cells_x": 50, "launcher_offset_x": 20},
    })
    assert problems == [
        "[ports.patterns] mating is set to its default ''; remove it",
        "[grid] capacity_cells_x is set to its default 50; remove it",
    ]
...
def test_configuration_round_trips_through_the_core_bytes() -> None:
    ...
    assert back.ports.patterns.mating == r"^Qb\d+\.port[2-5]$"
