...
def test_the_four_qubit_chip_loads_and_classifies() -> None:
    ...
    assert len(obstacles_of(chip)) == 207
    ports = ports_of(chip)
...
def test_the_nine_qubit_chip_loads_with_its_mating_ports() -> None:
    """The committed 9-qubit input classifies its mating ports and accepts its sequences."""
...
def test_role_names_follow_the_schema() -> None:
    ...
    assert role_name(UnassignedRole.Mating) == "mating"
