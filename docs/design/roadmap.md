...
The eight benchmark chips are 4, 9, 17, 21, 33, 45, 57 and 69 qubits. The
prototype also carries a 28-qubit and a 39-qubit driver. Both are experiments,
and both are out of scope for the port.
...
- All eight benchmark chips route with **zero unresolved wires** in the final
  stage.
- The 69-qubit benchmark completes in **no more than about 370 seconds**, which
  is the prototype's measured time at commit `42bccf4`.
- Total feedline angle cost is within an agreed tolerance of 196 on the 69-qubit
  benchmark.
- The assignment objective on the 69-qubit benchmark reproduces 132.68.
- Any run that establishes or checks these numbers records the `port_detection`
  mode that produced it. The figures above come from a run that derived the port
  ring from geometry; the shipped configurations default to the hand-written
  sequences, and the two modes are not yet proven to agree. See risk 6.
...
- Write `schemas/*.fbs` and the `nox -s schemas` session that regenerates the
  committed C++ and Python bindings. The configuration schema carries
  `port_detection` and `start_component` from the start, so phase 1 has
...
- Pattern-based role classification, with the classification table printed by
  `mqt-scpd doctor` so a wrong regex is visible before a six-minute run.
- The outer-boundary walk, behind `port_detection = "auto"`. `mqt-scpd doctor`
  prints the resolved ring, and where a configuration supplies a sequence as
  well, diffs the two and reports a rotation as a rotation. See
...
on every one under both detection modes, `mqt-scpd plot --stage layout` renders
each chip under 2 MB, `mqt-scpd render -o chip.gds` renders the *unrouted* chip,
and `mqt-scpd inspect` round-trips every artifact.
...
- `MQT::ScpdDrc` with DRCPolice's five **active** rules — wire clearance,
  feedline orthogonality, obstacle clearance, resonator length and wire loop —
  in both coordinate views, plus `drc.json` and `mqt-scpd drc`.
- Port the obstacle keepout as well as the obstacle rule. Upstream bakes it into
  the raster mask as an exact world-coordinate distance test, which is what
  makes the constraint hard rather than advisory.
...
clearance. Three of the five rules are ports of prototype code, including the
wire-loop check the prototype gained in commit `8a188ec` and shares with its own
router; see [decision 0024](decisions/0024-wire-loop-is-active.md). The other
two are the ones the prototype never had — obstacle clearance is enforced during
the search but never verified afterwards, and `lengthSatisfied` is a stub that
reads `false` everywhere.

## Phase 5 — Quality, tooling, documentation

- Metrics, `report.py`, `mqt-scpd benchmark`.
- DRCPolice's three **advisory** rules — component overlap, minimum straight
  length and minimum bend radius — with `--drc-all` and the `--drc` plot
  overlay. Measure what they report on all eight benchmarks before proposing any
  of them for promotion to active.
...
1. **The two outer port sequences are hand-maintained input by default.**
   `all_outer` runs to about 330 entries on the 69-qubit chip, and a typo in it
   is a silently worse assignment rather than a compile error. The risk is
   smaller than it was, because `port_detection = "auto"` can now derive the
   same sequence from geometry, but it does not disappear: manual stays the
   default, so the sequences still ship in the configurations. Mitigation:
   load-time validation that every label exists and matches exactly one role
   pattern, that `fixed_outer ⊆ all_outer`, `mqt-scpd doctor` printing the
   resolved ring, and `doctor` diffing a configured sequence against the walk
...
6. **The two port-detection modes are not proven to agree.** The acceptance
   numbers above come from a run with the ring derived from geometry. Against
   the previously published figures for the same chip, runtime fell from 456.1 s
   to 366.5 s and angle cost rose from 188 to 196. Three upstream changes landed
   between the two runs — the walk, the obstacle keepout and the loop protection
   — and the delta is attributed to none of them. No run of the current
   prototype exists in manual mode at all. Mitigation: measure both modes on all
   eight benchmarks in phase 1, before any of the routing phases treat either
   number as a baseline, and record which mode produced every figure that is
   quoted.
