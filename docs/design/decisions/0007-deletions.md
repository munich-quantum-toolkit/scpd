...
The prototype is roughly 69,800 lines, of which about 15,000 to 18,000 is real
...
| The clockwise port-ordering subsystem               |   ~350 | `all_outer` / `fixed_outer`, or the outer-boundary walk               |
| Ten `simple_<N>q_layout.cpp` drivers                | ~2,600 | The CLI and one `config.toml` per chip                                |
| 75 `getenv` sites and their branches                |   ~200 | `config.toml`; `FG_CLEARANCE_DEBUG` alone survives as `--drc-verbose` |
...
unrelated mechanism for the same output — a geometric outer-boundary walk of
about 740 lines — and that one is ported. See
[decision 0023](0023-geometric-port-ring-detection.md).
...
a value is slow, which is a real problem. Rejected anyway: 75 of them, each an
...
One of the 75 is kept, as a flag rather than a variable. `FG_CLEARANCE_DEBUG`
...
- Nothing that survives parses a coupler's qubit pair out of its name. The
  deleted port-ordering subsystem was its only consumer, and the walk that
  replaces it reads geometry instead, so the entity model goes either way
