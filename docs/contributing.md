...
[pull requests](https://github.com/munich-quantum-toolkit/scpd/pulls).
...
- 🐛 Report bugs:
  ...
  <https://github.com/munich-quantum-toolkit/scpd/issues>.
...
[GitHub Actions page](https://github.com/munich-quantum-toolkit/scpd/actions/workflows/ci.yml).
...
you can pass {code}`-DBUILD_MQT_SCPD_TESTS=OFF` to the CMake
configure step.
:::

Our CI pipeline on GitHub also collects code coverage information
and uploads it to
[Codecov](https://codecov.io/gh/munich-quantum-toolkit/scpd).
...
pass {code}`-DBUILD_MQT_SCPD_BINDINGS=ON` to the CMake configure
step.
...
{code}`python/mqt/scpd` directory.
...
MQT SCPD adheres to [Semantic Versioning],
...
Use it to explain how users may need to adapt their usage of MQT SCPD,
...
When it is time to release a new version of MQT SCPD,
...
changes for MQT SCPD, you can remove the {code}`minor` or {code}`major`
label from the PR.
...
[Releases page](https://github.com/munich-quantum-toolkit/scpd/releases) on
