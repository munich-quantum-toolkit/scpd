...
MQT SCPD is primarily developed as a C++20 library with Python bindings.
The Python package is available on [PyPI](https://pypi.org/project/mqt.scpd/) and can be installed on all major operating systems with all [officially supported Python versions](https://devguide.python.org/versions/).
...
```console
(.venv) $ python -m pip install mqt.scpd
```
...
```console
(.venv) $ python -c "import mqt.scpd; print(mqt.scpd.__version__)"
```
...
```console
$ uv pip install mqt.scpd --no-binary mqt.scpd
```
...
```console
(.venv) $ pip install mqt.scpd --no-binary mqt.scpd
```
...
To use the MQT SCPD Python package in your project, add it as a dependency in your {code}`pyproject.toml` or {code}`setup.py`.
...
setup(
    # ...
    install_requires=["mqt.scpd>=<version>"],
    # ...
)
```
...
- install MQT SCPD on your system and use CMake's {code}`find_package()` command to locate it, or
- use CMake's [{code}`FetchContent`][FetchContent] module to combine both approaches.

::::{tab-set}
:::{tab-item} {code}`FetchContent`

This is the recommended approach because it lets you detect installed versions of MQT SCPD and only downloads the library if it is not available on the system.
...
# cmake-format: off
set(MQT_SCPD_MINIMUM_VERSION "<minimum_version>"
    CACHE STRING "MQT SCPD minimum version")
set(MQT_SCPD_VERSION "<version>"
    CACHE STRING "MQT SCPD version")
set(MQT_SCPD_REV "<revision>"
    CACHE STRING "MQT SCPD identifier (tag, branch or commit hash)")
set(MQT_SCPD_REPO_OWNER "munich-quantum-toolkit"
    CACHE STRING "MQT SCPD repository owner (change when using a fork)")
# cmake-format: on
FetchContent_Declare(
  mqt-scpd
  GIT_REPOSITORY https://github.com/${MQT_SCPD_REPO_OWNER}/scpd.git
  GIT_TAG ${MQT_SCPD_REV}
  FIND_PACKAGE_ARGS ${MQT_SCPD_MINIMUM_VERSION})
list(APPEND FETCH_PACKAGES mqt-scpd)
...
```console
$ git submodule add https://github.com/munich-quantum-toolkit/scpd.git external/mqt-scpd
```

Then add the following line to your {code}`CMakeLists.txt` to make the library's targets available in your project:

```cmake
add_subdirectory(external/mqt-scpd)
```
...
You can install MQT SCPD on your system after building it from source:

```console
$ git clone https://github.com/munich-quantum-toolkit/scpd.git mqt-scpd
$ cd mqt-scpd
$ cmake -S . -B build
$ cmake --build build
$ cmake --install build
...
```cmake
find_package(mqt-scpd <version> REQUIRED)
```
...
Set up a reproducible development environment for MQT SCPD.
...
1.  Get the code:
    ...
    If you do not have write access to the [munich-quantum-toolkit/scpd](https://github.com/munich-quantum-toolkit/scpd) repository, fork the repository on GitHub (see <https://docs.github.com/en/get-started/quickstart/fork-a-repo>) and clone your fork locally.

    ```console
    $ git clone git@github.com:your_name_here/scpd.git mqt-scpd
    ```

    :::
    :::{tab-item} Internal Contribution
    If you have write access to the [munich-quantum-toolkit/scpd](https://github.com/munich-quantum-toolkit/scpd) repository, clone the repository locally.

    ```console
    $ git clone git@github.com/munich-quantum-toolkit/scpd.git mqt-scpd
    ```
