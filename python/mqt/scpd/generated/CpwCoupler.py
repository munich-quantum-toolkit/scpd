# automatically generated by the FlatBuffers compiler, do not modify

# namespace: generated

import flatbuffers
from flatbuffers.compat import import_numpy
from typing import Any
from mqt.scpd.generated.Point import Point
from mqt.scpd.generated.PortRef import PortRef
from typing import Optional
np = import_numpy()

# A coplanar-waveguide coupler that taps a feedline. Instantiated by the
# Final stage at exact physical dimensions in layout units.
class CpwCoupler(object):
    __slots__ = ['_tab']

    @classmethod
    def GetRootAs(cls, buf, offset: int = 0):
        n = flatbuffers.encode.Get(flatbuffers.packer.uoffset, buf, offset)
        x = CpwCoupler()
        x.Init(buf, n + offset)
        return x

    @classmethod
    def GetRootAsCpwCoupler(cls, buf, offset=0):
        """This method is deprecated. Please switch to GetRootAs."""
        return cls.GetRootAs(buf, offset)
    # CpwCoupler
    def Init(self, buf: bytes, pos: int):
        self._tab = flatbuffers.table.Table(buf, pos)

    # The ResonatorSource port the coupler carries.
    # CpwCoupler
    def Port(self) -> Optional[PortRef]:
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(4))
        if o != 0:
            x = o + self._tab.Pos
            obj = PortRef()
            obj.Init(self._tab.Bytes, x)
            return obj
        return None

    # CpwCoupler
    def Centre(self) -> Optional[Point]:
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(6))
        if o != 0:
            x = o + self._tab.Pos
            obj = Point()
            obj.Init(self._tab.Bytes, x)
            return obj
        return None

    # CpwCoupler
    def Rotation(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(8))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Uint8Flags, o + self._tab.Pos)
        return 0

    # CpwCoupler
    def Length(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(10))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Float64Flags, o + self._tab.Pos)
        return 0.0

    # CpwCoupler
    def Height(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(12))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Float64Flags, o + self._tab.Pos)
        return 0.0

def CpwCouplerStart(builder: flatbuffers.Builder):
    builder.StartObject(5)

def Start(builder: flatbuffers.Builder):
    CpwCouplerStart(builder)

def CpwCouplerAddPort(builder: flatbuffers.Builder, port: Any):
    builder.PrependStructSlot(0, flatbuffers.number_types.UOffsetTFlags.py_type(port), 0)

def AddPort(builder: flatbuffers.Builder, port: Any):
    CpwCouplerAddPort(builder, port)

def CpwCouplerAddCentre(builder: flatbuffers.Builder, centre: Any):
    builder.PrependStructSlot(1, flatbuffers.number_types.UOffsetTFlags.py_type(centre), 0)

def AddCentre(builder: flatbuffers.Builder, centre: Any):
    CpwCouplerAddCentre(builder, centre)

def CpwCouplerAddRotation(builder: flatbuffers.Builder, rotation: int):
    builder.PrependUint8Slot(2, rotation, 0)

def AddRotation(builder: flatbuffers.Builder, rotation: int):
    CpwCouplerAddRotation(builder, rotation)

def CpwCouplerAddLength(builder: flatbuffers.Builder, length: float):
    builder.PrependFloat64Slot(3, length, 0.0)

def AddLength(builder: flatbuffers.Builder, length: float):
    CpwCouplerAddLength(builder, length)

def CpwCouplerAddHeight(builder: flatbuffers.Builder, height: float):
    builder.PrependFloat64Slot(4, height, 0.0)

def AddHeight(builder: flatbuffers.Builder, height: float):
    CpwCouplerAddHeight(builder, height)

def CpwCouplerEnd(builder: flatbuffers.Builder) -> int:
    return builder.EndObject()

def End(builder: flatbuffers.Builder) -> int:
    return CpwCouplerEnd(builder)

import mqt.scpd.generated.Point
import mqt.scpd.generated.PortRef
try:
    from typing import Optional
except:
    pass

class CpwCouplerT(object):

    # CpwCouplerT
    def __init__(
        self,
        port = None,
        centre = None,
        rotation = 0,
        length = 0.0,
        height = 0.0,
    ):
        self.port = port  # type: Optional[mqt.scpd.generated.PortRef.PortRefT]
        self.centre = centre  # type: Optional[mqt.scpd.generated.Point.PointT]
        self.rotation = rotation  # type: int
        self.length = length  # type: float
        self.height = height  # type: float

    @classmethod
    def InitFromBuf(cls, buf, pos):
        cpwCoupler = CpwCoupler()
        cpwCoupler.Init(buf, pos)
        return cls.InitFromObj(cpwCoupler)

    @classmethod
    def InitFromPackedBuf(cls, buf, pos=0):
        n = flatbuffers.encode.Get(flatbuffers.packer.uoffset, buf, pos)
        return cls.InitFromBuf(buf, pos+n)

    @classmethod
    def InitFromObj(cls, cpwCoupler):
        x = CpwCouplerT()
        x._UnPack(cpwCoupler)
        return x

    # CpwCouplerT
    def _UnPack(self, cpwCoupler):
        if cpwCoupler is None:
            return
        if cpwCoupler.Port() is not None:
            self.port = mqt.scpd.generated.PortRef.PortRefT.InitFromObj(cpwCoupler.Port())
        if cpwCoupler.Centre() is not None:
            self.centre = mqt.scpd.generated.Point.PointT.InitFromObj(cpwCoupler.Centre())
        self.rotation = cpwCoupler.Rotation()
        self.length = cpwCoupler.Length()
        self.height = cpwCoupler.Height()

    # CpwCouplerT
    def Pack(self, builder):
        CpwCouplerStart(builder)
        if self.port is not None:
            port = self.port.Pack(builder)
            CpwCouplerAddPort(builder, port)
        if self.centre is not None:
            centre = self.centre.Pack(builder)
            CpwCouplerAddCentre(builder, centre)
        CpwCouplerAddRotation(builder, self.rotation)
        CpwCouplerAddLength(builder, self.length)
        CpwCouplerAddHeight(builder, self.height)
        cpwCoupler = CpwCouplerEnd(builder)
        return cpwCoupler
