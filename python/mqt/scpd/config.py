...
def _read_ports(table: dict[str, Any], problems: list[str]) -> PortConfigT:
    ...
    if patterns_table is None:
        ...
        config.patterns.mating = patterns.take("mating", str)
        patterns.finish()
...
def shipped_config_problems(document: dict[str, Any]) -> list[str]:
    ...
    if isinstance(ports, dict):
        patterns = ports.get("patterns")
        if isinstance(patterns, dict) and patterns.get("mating") == "":
            problems.append("[ports.patterns] mating is set to its default ''; remove it")
