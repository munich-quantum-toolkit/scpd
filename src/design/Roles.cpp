...
/// The patterns in the order of the configuration keys. The mating pattern
/// takes part only when the configuration supplies one.
std::vector<CompiledPattern> compile(const PortPatternsT& patterns) {
  ...
  if (!patterns.mating.empty()) {
    add("mating", UnassignedRole::Mating, patterns.mating);
  }
  return compiled;
}
