...
struct CornerHash {
  [[nodiscard]] std::size_t operator()(const Corner& corner) const noexcept {
    return (static_cast<std::size_t>(static_cast<std::uint32_t>(corner.x)) << 32U) ^
...
std::optional<std::pair<std::uint32_t, std::uint32_t>> refinement(const GridMetrics& detail,
                                                                  const GridMetrics& coarse) {
  if (coarse.width == 0 || coarse.height == 0 || detail.width % coarse.width != 0 ||
      detail.height % coarse.height != 0) {
    return std::nullopt;
  }
...
Point PartitionBorder::center() const {
  ...
  return {x / static_cast<double>(samples.size()), y / static_cast<double>(samples.size())};
}

Partitions extractPartitions(const BitGrid& blocked, const std::span<const PartitionLabel> labels,
                             const GridMetrics& grid) {
  if (blocked.width() != grid.width || blocked.height() != grid.height ||
      labels.size() != grid.cells()) {
    throw std::invalid_argument(
        std::format("the mask is {}x{} cells and the labels {}, on a grid of {}x{}",
                    blocked.width(), blocked.height(), labels.size(), grid.width, grid.height));
  }
  ...
  const auto labelAt = [&](const std::int64_t x, const std::int64_t y) -> PartitionLabel {
    if (x < 0 || y < 0 || x >= static_cast<std::int64_t>(grid.width) ||
        y >= static_cast<std::int64_t>(grid.height)) {
      return LABEL_NONE;
    }
    const auto index = (static_cast<std::size_t>(y) * grid.width) + static_cast<std::size_t>(x);
  ...
  const auto addBorder = [&](const PartitionLabel a, const PartitionLabel b, const double x,
                             const double y) {
    ...
    if (latticeKeys.insert((static_cast<std::uint64_t>(qx) << 32U) | qy).second) {
  ...
  for (std::uint32_t y = 0; y < grid.height; ++y) {
    for (std::uint32_t x = 0; x < grid.width; ++x) {
      ...
      if (const auto right = labelAt(ix + 1, iy); right != LABEL_NONE && right != label) {
        addBorder(label, right, static_cast<double>(x + 1), static_cast<double>(y) + 0.5);
      }
      if (const auto below = labelAt(ix, iy + 1); below != LABEL_NONE && below != label) {
        addBorder(label, below, static_cast<double>(x) + 0.5, static_cast<double>(y + 1));
      }
  ...
  for (auto& [label, edges] : outlineEdges) {
    ...
    while (!outgoing.empty()) {
      ...
      // outline can have.
      if (ring.size() >= 4) {
        partitions.outlines.push_back({.label = label, .ring = std::move(ring)});
      }
    }
  }
...
std::vector<std::size_t> freeCellSeeds(const BitGrid& blocked, const GridMetrics& detail,
                                       const GridMetrics& coarse) {
  const auto factors = refinement(detail, coarse);
  if (!factors.has_value()) {
    throw std::invalid_argument(std::format(
        "a detail grid of {}x{} cells is no whole refinement of a coarse grid of {}x{}",
        detail.width, detail.height, coarse.width, coarse.height));
  }
  if (blocked.width() != detail.width || blocked.height() != detail.height) {
    throw std::invalid_argument(std::format("the mask is {}x{} cells and the detail grid {}x{}",
                                            blocked.width(), blocked.height(), detail.width,
                                            detail.height));
  }
  ...
  for (std::uint32_t cy = 0; cy < coarse.height; ++cy) {
    for (std::uint32_t cx = 0; cx < coarse.width; ++cx) {
      ...
      if (clear) {
        seeds.push_back(detail.index(x0 + (cellWidth / 2), y0 + (cellHeight / 2)));
      }
    }
  }
  return seeds;
...
std::vector<CellCapacity> cellCapacities(const BitGrid& blocked, const GridMetrics& detail,
                                         const GridMetrics& coarse, const double wirePitch) {
  const auto factors = refinement(detail, coarse);
  if (!factors.has_value()) {
    throw std::invalid_argument(std::format(
        "a detail grid of {}x{} cells is no whole refinement of a coarse grid of {}x{}",
  ...
  if (wirePitch <= 0.0) {
    throw std::invalid_argument(std::format("a wire pitch of {} is not positive", wirePitch));
  }
  const auto [cellWidth, cellHeight] = *factors;
